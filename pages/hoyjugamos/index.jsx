...
const VideojuegosPage = ({ posts }) => {
  const categoryPosts = posts.filter(
    (post) => post.category.replace(/\s+/g, '').toLowerCase() === 'hoyjugamos'
  )
  // const categoryPosts = posts.filter((post) => post.category === 'hoyjugamos')
  const category = 'Hoy Jugamos'
  return (
    <MainLayout
      title={SEO.title}
      url={SEO.url}
